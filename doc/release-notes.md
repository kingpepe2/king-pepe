...
KingPepe Core version 25.0 is now available from:

  <https://bitcoincore.org/bin/kingpepe-core-25.0/>
...
- All JSON-RPC methods accept a new [named
  ...
  python test framework and `kingpepe-cli` tool both take advantage of this, so
  for example:

```sh
kingpepe-cli -named createwallet wallet_name=mywallet load_on_startup=1
```

Can now be shortened to:

```sh
kingpepe-cli -named createwallet mywallet load_on_startup=1
```
...
dbcache, KingPepe Core will now return an error at startup. (#25574)
...
before KingPepe Core has begun its shutdown sequence. (#23395)
...
- In the `createwallet`, `loadwallet`, `unloadwallet`, and `restorewallet` RPCs, the
  ...
  deprecation period by launching kingpeped with the configuration option
...
- The binary verification script has been updated. In previous releases it
  ...
  examples, see:
  https://github.com/kingpepe/kingpepe/blob/master/contrib/verify-binaries/README.md
  (#27358)
