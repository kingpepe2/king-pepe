...
const char * const KINGPEPE_CONF_FILENAME = "kingpepe.conf";
const char * const KINGPEPE_SETTINGS_FILENAME = "settings.json";
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //kingpepe-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", KINGPEPE_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\KingPepe
    // macOS: ~/Library/Application Support/KingPepe
    // Unix-like: ~/.kingpepe
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "KingPepe";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/KingPepe";
#else
    // Unix-like
    return pathRet / ".kingpepe";
#endif
#endif
}
...
{
    return GetConfigFile(*this, GetPathArg("-conf", KINGPEPE_CONF_FILENAME));
}
