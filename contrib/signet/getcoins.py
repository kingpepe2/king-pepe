...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing kingpepe-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='kingpepe-cli', help='kingpepe-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='KingPepe address to which the faucet should send')
...
parser.add_argument('kingpepe_cli_args', nargs='*', help='Arguments to pass on to kingpepe-cli (default: -signet)')

args = parser.parse_args()

if args.kingpepe_cli_args == []:
    args.kingpepe_cli_args = ['-signet']


def kingpepe_cli(rpc_command_and_params):
    argv = [args.cmd] + args.kingpepe_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = kingpepe_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = kingpepe_cli(['getnewaddress', 'faucet', 'bech32'])
