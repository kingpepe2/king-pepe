...
""" Example logging KingPepe Core mempool events using the mempool:added,
...
def main(kingpeped_path):
    kingpeped_with_usdts = USDT(path=str(kingpeped_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    kingpeped_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    kingpeped_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    kingpeped_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    kingpeped_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[kingpeped_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/kingpeped")
        exit(1)
