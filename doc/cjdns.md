# CJDNS support in KingPepe Core

It is possible to run KingPepe Core over CJDNS, an encrypted IPv6 network that
...
done outside of KingPepe Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the KingPepe network and individual nodes.
...
before it will work with your KingPepe Core node. This is described in steps
...
## Run KingPepe Core with CJDNS

Once you are connected to the CJDNS network, the following KingPepe Core
...
When enabled, this option tells KingPepe Core that it is running in an
...
IPv6 local network. This helps KingPepe Core perform better address management:
...
CJDNS support was added to KingPepe Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `kingpepe-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/kingpepe/kingpepe/blob/master/doc/tor.md) for
details.

## CJDNS-related information in KingPepe Core

There are several ways to see your CJDNS address in KingPepe Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `kingpepe-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `kingpepe-cli getpeerinfo`).
