...
class ConfArgsTest(KingPepeTestFramework):
    ...
    def test_config_file_parser(self):
        ...
        # Check that startup fails if conf= is set in kingpepe.conf or in an included conf file
        bad_conf_file_path = os.path.join(self.options.tmpdir, 'node0', 'kingpepe_bad.conf')
        ...
        with open(os.path.join(self.nodes[0].datadir, 'kingpepe.conf'), 'a', encoding='utf-8') as conf:
        ...
        main_conf_file_path = os.path.join(self.options.tmpdir, 'node0', 'kingpepe_main.conf')
    ...
    def run_test(self):
        ...
        conf_file = os.path.join(default_data_dir, "kingpepe.conf")
        ...
        none_existent_conf_file = os.path.join(default_data_dir, "none_existent_kingpepe.conf")
