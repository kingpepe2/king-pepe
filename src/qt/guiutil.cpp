...
#include <qt/kingpepeaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a KingPepe address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new KingPepeAddressEntryValidator(parent));
    widget->setCheckValidator(new KingPepeAddressCheckValidator(parent));
}
...
bool parseKingPepeURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no kingpepe: URI
    if(!uri.isValid() || uri.scheme() != QString("kingpepe"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!KingPepeUnits::parse(KingPepeUnit::KPEPE, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseKingPepeURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseKingPepeURI(uriInstance, out);
}

QString formatKingPepeURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("kingpepe:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(KingPepeUnits::format(KingPepeUnit::KPEPE, info.amount, false, KingPepeUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openKingPepeConf()
{
    ...
    /* Open kingpepe.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "KingPepe.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "KingPepe (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("KingPepe (%s).lnk", chain));
}

bool GetStartOnSystemStartup()
{
    // check for KingPepe*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "kingpepe.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("kingpepe-%s.desktop", chain));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a kingpepe.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=KingPepe\n";
        else
            optionFile << strprintf("Name=KingPepe (%s)\n", chain);
