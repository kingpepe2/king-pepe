KingPepe Core version 0.18.0 is now available from:

  <https://bitcoincore.org/bin/kingpepe-core-0.18.0/>
...
`/Applications/KingPepe-Qt` (on Mac) or `kingpeped`/`kingpepe-qt` (on
Linux).
...
recommended to use KingPepe Core on unsupported systems.

KingPepe Core should also work on most other Unix-like systems but is not
as frequently tested on them.

From 0.17.0 onwards, macOS <10.10 is no longer supported. 0.17.0 is
built using Qt 5.9.x, which doesn't support versions of macOS older than
10.10. Additionally, KingPepe Core does not yet change appearance when
...
`contrib/init/kingpeped.service`, it has been changed to use
`/var/lib/kingpeped` as the data directory instead of
`~kingpepe/.kingpepe`. When switching over to the new configuration file,
please make sure that the filesystem on which `/var/lib/kingpeped` will
exist has enough space (check using `df -h /var/lib/kingpeped`), and
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/kingpepe/kingpepe/blob/master/doc/zmq.md#usage)
  for details.
...
- The `rpcallowip` option can no longer be used to automatically listen
  ...
  [...] -p 127.0.0.1:22093:22093` (this is an extra `:22093` over the
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](https://bitcoincore.org/en/contact/) the KingPepe Core
...
The systemd init file (`contrib/init/kingpeped.service`) has been changed
to use `/var/lib/kingpeped` as the data directory instead of
`~kingpepe/.kingpepe`. This change makes KingPepe Core more consistent with
...
`datadir` option in `/etc/kingpepe/kingpepe.conf` will have no effect.
This is because the command line arguments specified in the init files
take precedence over the options specified in
`/etc/kingpepe/kingpepe.conf`.


Documentation
-------------

- A new short [document](https://github.com/kingpepe/kingpepe/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/kingpepe/kingpepe/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/kingpepe/kingpepe/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/kingpepe/kingpepe/blob/master/doc/kingpepe-conf.md)
  about the `kingpepe.conf` file describes how to use it to configure
  KingPepe Core.

- A new document introduces KingPepe Core's BIP174 [Partially-Signed
  KingPepe Transactions
  (PSBT)](https://github.com/kingpepe/kingpepe/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/kingpepe/kingpepe/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent KingPepe-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  KingPepe Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/kingpepe/kingpepe/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/kingpepe/kingpepe/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when KingPepe Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of KingPepe Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/kingpepe/kingpepe/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/kingpepe/kingpepe/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart kingpeped with the `-deprecatedrpc=generate`
...
- A new `kingpepe-wallet` tool is now distributed alongside KingPepe
...
This section describes planned changes to KingPepe Core that may affect
other KingPepe software and services.

- Since version 0.16.0, KingPepe Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with KingPepe Core 0.20 (expected about
  a year after 0.18), KingPepe Core will default to native segwit
  ...
  to bech32 addresses, and if the KingPepe Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in KingPepe Core 0.19 (approximately November 2019).
  ...
  set the `addresstype=bech32` configuration option in any KingPepe Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, KingPepe Core
...
- A sub-project of KingPepe Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with KingPepe Core.  See their [project
  page](https://github.com/kingpepe-core/HWI#readme) for details.

Security
--------

- This release changes the Random Number Generator (RNG) used from
  OpenSSL to KingPepe Core's own implementation, although entropy
  gathered by KingPepe Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves KingPepe Core a
...
- On macOS, KingPepe Core now opts out of application CPU throttling
...
- #12490 Remove deprecated wallet rpc features from `kingpepe_server` (jnewbery)
- #14138 Set `encrypted_batch` to nullptr after delete. Avoid double free in the case of NDEBUG (practicalswift)
- #14168 Remove `ENABLE_WALLET` from `libkingpepe_server.a` (jnewbery)
...
- #13926 [Tools] kingpepe-wallet - a tool for creating and managing wallets offline (jnewbery)
...
- #14162 Also log and print messages or questions like kingpeped (MarcoFalke)
...
- #14521 qt, docs: Fix `kingpepe-qt -version` output formatting (hebasto)
...
- #11625 Add KingPepeApplication & RPCConsole tests (ryanofsky)
...
- #14204 Move `interfaces/*` to `libkingpepe_server` (laanwj)
...
- #15154 Configure: kingpepe-tx doesn't need libevent, so don't pull it in (luke-jr)
...
- #15388 Makefile.am: add rule for src/kingpepe-wallet (Sjors)
...
- #11526 Visual Studio build configuration for KingPepe Core (sipsorcery)
- #15110 build\_msvc: Fix the build problem in `libkingpepe_server` (Mr-Leshiy)
...
- #13953 Fix deprecation in kingpepe-util-test.py (isghe)
...
- #12775 Integration of property based testing into KingPepe Core (Christewart)
...
- #12246 Bugfix: Only run kingpepe-tx tests when kingpepe-tx is enabled (luke-jr)
...
- #14092 Dry run `bench_kingpepe` as part `make check` to allow for quick identification of assertion/sanitizer failures in benchmarking code (practicalswift)
...
- #14985 Remove `thread_local` from `test_kingpepe` (MarcoFalke)
...
- #14186 kingpepe-cli: don't translate command line options (HashUnlimited)
- #14057 logging: Only log `using config file path_to_kingpepe.conf` message on startup if conf file exists (leishman)
...
- #12255 Update kingpepe.service to conform to init.md (dongcarl)
...
- #14497 Add doc/kingpepe-conf.md (hebasto)
...
- #15354 Add missing `kingpepe-wallet` tool manpages (MarcoFalke)
...
- #15765 correct kingpepeconsensus_version in shared-libraries.md (fanquake)
- #15792 describe onlynet option in doc/tor.md (jonatack)
- #15802 mention creating application support kingpepe folder on OSX (JimmyMow)
...
As well as everyone that helped translating on [Transifex](https://www.transifex.com/projects/p/kingpepe/).
