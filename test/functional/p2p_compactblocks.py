...
# TestP2PConn: A peer we use to send messages to kingpeped, and store responses.
...
class CompactBlocksTest(KingPepeTestFramework):
    ...
    # This test actually causes kingpeped to (reasonably!) disconnect us, so do this last.
    ...
    # Test that kingpeped requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for kingpeped to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        node = self.nodes[0]
        # kingpeped will not send blocktxn responses for blocks whose height is
