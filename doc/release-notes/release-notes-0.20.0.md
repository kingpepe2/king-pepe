...
KingPepe Core version 0.20.0 is now available from:

  <https://bitcoincore.org/bin/kingpepe-core-0.20.0/>
...
- Instead of running `make` simply, you should instead run
  `KINGPEPE_GENBUILD_NO_GIT=1 make`.
...
#### Removal of BIP61 reject network messages from KingPepe Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since KingPepe Core version 0.18.0.
...
- Testing or debugging of implementations of the KingPepe P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of KingPepe Core. KingPepe Core logs debug messages
...
- OpenSSL is no longer used by KingPepe Core.  (#17265)

- BIP70 support has been fully removed from KingPepe Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/kingpepe/kingpepe/issues/16599),
  [PR #16702](https://github.com/kingpepe/kingpepe/pull/16702), and the
  `kingpeped help` for more information.  This option is experimental and
...
- All custom settings configured when KingPepe Core starts are now
...
- The "Start KingPepe Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://bitcoincore.org/en/releases/0.18.0/#wallet-gui)
  ...
  affected anyone who simultaneously used multiple KingPepe Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed KingPepe Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/kingpepe-core/HWI) for
  signing.  Future versions of KingPepe Core should support a GUI option
...
- KingPepe Core's automatically-generated source code documentation is
  now available at https://doxygen.kingpepecore.org.  (#17596)
...
- The `kingpepe-cli` utility used with the `-getinfo` parameter now
...
- The `kingpepe-cli` utility now accepts a `-stdinwalletpassphrase`
...
- Command line options prefixed with main/test/regtest network names like
  `-main.port=22094` `-test.server=1` previously were allowed but ignored. Now
...
- #17068 Always generate `kingpepestrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_kingpepe-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBKINGPEPE_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_kingpepe` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBKINGPEPE_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for kingpepe-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate kingpepe schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest kingpepe.conf (sdaftuar)
...
- #18022 Fix appveyor `test_kingpepe` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (kingpepe-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone kingpepe-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about KingPepe Core files (hebasto)
...
- #17119 Fix broken kingpepe-cli examples (andrewtoth)
...
- #17503 Remove bitness from kingpepe-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.kingpepecore.org (laanwj)
...
- #17648 Rename wallet-tool references to kingpepe-wallet (hel-o)
...
- #18341 Replace remaining literal KPEPE with `CURRENCY_UNIT` (domob1812)
...
- #16115 On kingpeped startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from kingpepe-cli -getinfo (malevolent)
