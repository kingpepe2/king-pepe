# User-space, Statically Defined Tracing (USDT) for KingPepe Core

KingPepe Core includes statically defined tracepoints to allow for more
...
**Note**: KingPepe Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to KingPepe Core
...
Multiple tools can list the available tracepoints in a `kingpeped` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in KingPepe Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/kingpeped
stap net        outbound_message 0x0000000000107c05 /src/kingpeped
stap validation block_connected  0x00000000002fb10c /src/kingpeped
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in KingPepe Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/kingpeped | grep NT_STAPSDT -A 4 -B 2
