...
The KingPepe Core daemon can be configured to act as a trusted "border
router", implementing the kingpepe wire protocol and relay, making
...
The ZeroMQ feature in KingPepe Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building kingpeped:
...
For instance:
    ...
    $ kingpeped -zmqpubhashtx=tcp://127.0.0.1:222093 \
               -zmqpubhashtx=tcp://192.168.1.2:222093 \
               -zmqpubhashblock="tcp://[::1]:222094" \
               -zmqpubrawtx=ipc:///tmp/kingpeped.tx.raw \
               -zmqpubhashtxhwm=10000

Each PUB notification has a topic and body, where the header
corresponds to the notification type. For instance, for the
notification `-zmqpubhashtx` the topic is `hashtx` (no null
terminator). These options can also be provided in kingpepe.conf.
...
From the perspective of kingpeped, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into kingpeped directly. Furthermore, no information is
...
using. KingPeped appends an up-counting sequence number to each
