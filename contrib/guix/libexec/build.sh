...
(
    ...
    case "$HOST" in
        *mingw*)
            make deploy ${V:+V=1} KINGPEPE_WIN_INSTALLER="${OUTDIR}/${DISTNAME}-win64-setup-unsigned.exe"
            ;;
    esac

    # Setup the directory where our KingPepe Core build for HOST will be
    # installed. This directory will also later serve as the input for our
    # binary tarballs.
    INSTALLPATH="${PWD}/installed/${DISTNAME}"
    mkdir -p "${INSTALLPATH}"
    # Install built KingPepe Core to $INSTALLPATH
    case "$HOST" in
        *darwin*)
    ...
    (
        ...
        # copy over the example kingpepe.conf file. if contrib/devtools/gen-kingpepe-conf.sh
        # has not been run before buildling, this file will be a stub
        cp "${DISTSRC}/share/examples/kingpepe.conf" "${DISTNAME}/"
