...
public:
    ...
    QString describe(interfaces::Node& node, interfaces::Wallet& wallet, TransactionRecord* rec, KingPepeUnit unit)
    {
        ...
        platformStyle(_platformStyle)
{
    ...
    columns << QString() << QString() << tr("Date") << tr("Type") << tr("Label") << KingPepeUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
{
    columns[Amount] = KingPepeUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
QString TransactionTableModel::formatTxAmount(const TransactionRecord *wtx, bool showUnconfirmed, KingPepeUnits::SeparatorStyle separators) const
{
    QString str = KingPepeUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->credit + wtx->debit, false, separators);
    if(showUnconfirmed)
    {
...
QVariant TransactionTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DisplayRole:
        ...
        case Amount:
            return formatTxAmount(rec, true, KingPepeUnits::SeparatorStyle::ALWAYS);
        } // no default case, so the compiler can warn about missing cases
        assert(false);
    case Qt::EditRole:
    ...
    case TxPlainTextRole:
        {
            ...
            details.append(formatTxAmount(rec, false, KingPepeUnits::SeparatorStyle::NEVER));
            return details;
        }
    case ConfirmedRole:
    ...
    case FormattedAmountRole:
        ...
        return formatTxAmount(rec, false, KingPepeUnits::SeparatorStyle::NEVER);
    case StatusRole:
