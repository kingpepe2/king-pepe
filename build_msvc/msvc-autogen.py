...
def parse_config_into_kpepe_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"KingPepe Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/kingpepe_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/kingpepe_config.h'), "w", encoding="utf8") as kpepe_config:
        kpepe_config.writelines(template)
...
def main():
    parser = argparse.ArgumentParser(description='KingPepe-core msbuild configuration initialiser.')
    ...
    parse_config_into_kpepe_config()
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/kingpepe_config.h'), os.path.join(SOURCE_DIR, 'config/kingpepe-config.h'))
