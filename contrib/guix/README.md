# Bootstrappable KingPepe Core Builds

This directory contains the files necessary to perform bootstrappable KingPepe
Core builds.
...
To build KingPepe Core reproducibly with all default options, invoke the
...
2. The `guix-codesign` command ***requires*** a `DETACHED_SIGS_REPO` flag.
    * _**DETACHED_SIGS_REPO**_
      ...
      KingPepe Core version being built.
...
```
env DETACHED_SIGS_REPO=<path/to/kingpepe-detached-sigs> ./contrib/guix/guix-codesign
```
...
repository](https://github.com/kingpepe-core/guix.sigs).
