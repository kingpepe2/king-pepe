...
Below are some notes on how to build KingPepe Core for Windows.

The options known to work for building KingPepe Core on Windows are:
...
Note that for WSL the KingPepe Core source path MUST be somewhere in the default mount file system, for
example /usr/src/kingpepe, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\kingpepe`, for example:

    make install DESTDIR=/mnt/c/workspace/kingpepe
