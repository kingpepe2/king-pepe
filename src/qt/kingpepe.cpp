...
#include <config/kingpepe-config.h>
#endif

#include <qt/kingpepe.h>
...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<KingPepeUnit>("KingPepeUnit");
#else
    qRegisterMetaType<KingPepeUnit>("KingPepeUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. kingpepe_de.qm (shortcut "de" needs to be defined in kingpepe.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. kingpepe_de_DE.qm (shortcut "de_DE" needs to be defined in kingpepe.qrc)
...
static const char* qt_argv = "kingpepe-qt";

KingPepeApplication::KingPepeApplication()
...
void KingPepeApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the KingPepeApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", KingPepeGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void KingPepeApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool KingPepeApplication::createOptionsModel(bool resetSettings)
{
...
void KingPepeApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new KingPepeGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &KingPepeGUI::quitRequested, this, &KingPepeApplication::requestShutdown);
...
void KingPepeApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void KingPepeApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool KingPepeApplication::baseInitialize()
{
    return node().baseInitialize();
}

void KingPepeApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &KingPepeApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &KingPepeApplication::handleRunawayException);
    connect(this, &KingPepeApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &KingPepeApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void KingPepeApplication::parameterSetup()
{
...
void KingPepeApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void KingPepeApplication::requestInitialize()
{
...
void KingPepeApplication::requestShutdown()
{
...
void KingPepeApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if(success) {
        ...
        // kingpepe: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &KingPepeGUI::handlePaymentRequest);
            connect(window, &KingPepeGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void KingPepeApplication::handleRunawayException(const QString &message)
{
...
void KingPepeApplication::handleNonFatalException(const QString& message)
{
...
WId KingPepeApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool KingPepeApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", KingPepeGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    Q_INIT_RESOURCE(kingpepe);
    Q_INIT_RESOURCE(kingpepe_locale);
    ...
    /// 6-7. Parse kingpepe.conf, determine network, switch to network specific
    ...
    // kingpepe: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
