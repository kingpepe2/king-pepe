...
const std::string KingPepeGUI::DEFAULT_UIPLATFORM =
...
KingPepeGUI::KingPepeGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &KingPepeGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &KingPepeGUI::showModalOverlay);
...
void KingPepeGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a KingPepe address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and kingpepe: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &KingPepeGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &KingPepeGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &KingPepeGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your KingPepe addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified KingPepe addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed KingPepe Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed KingPepe Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a kingpepe: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible KingPepe command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &KingPepeGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &KingPepeGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &KingPepeGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &KingPepeGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &KingPepeGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &KingPepeGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &KingPepeGUI::setCurrentWallet, Qt::QueuedConnection);
                    activity->open(path);
                });
            }
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &KingPepeGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &CreateWalletActivity::created, this, &KingPepeGUI::setCurrentWallet);
            activity->create();
        });
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_mask_values_action, &QAction::toggled, this, &KingPepeGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &KingPepeGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &KingPepeGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &KingPepeGUI::showDebugWindow);
}

void KingPepeGUI::createMenuBar()
{
#ifdef Q_OS_MACOS
...
void KingPepeGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &KingPepeGUI::setCurrentWalletBySelectorIndex);
...
void KingPepeGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &KingPepeGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &KingPepeGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &KingPepeGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &KingPepeGUI::showProgress);
...
#ifdef ENABLE_WALLET
void KingPepeGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void KingPepeGUI::setWalletController(WalletController* wallet_controller)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &KingPepeGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &KingPepeGUI::removeWallet);
...
WalletController* KingPepeGUI::getWalletController()
{
    return m_wallet_controller;
}

void KingPepeGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &KingPepeGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &KingPepeGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &KingPepeGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &KingPepeGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &KingPepeGUI::incomingTransaction);
    connect(this, &KingPepeGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void KingPepeGUI::removeWallet(WalletModel* walletModel)
{
...
void KingPepeGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void KingPepeGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void KingPepeGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void KingPepeGUI::setWalletActionsEnabled(bool enabled)
{
...
void KingPepeGUI::createTrayIcon()
{
...
void KingPepeGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &KingPepeGUI::toggleHidden);
...
void KingPepeGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void KingPepeGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void KingPepeGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void KingPepeGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void KingPepeGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void KingPepeGUI::openClicked()
{
...
void KingPepeGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void KingPepeGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void KingPepeGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void KingPepeGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void KingPepeGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void KingPepeGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void KingPepeGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void KingPepeGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to KingPepe network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void KingPepeGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void KingPepeGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void KingPepeGUI::updateHeadersSyncProgressLabel()
{
...
void KingPepeGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void KingPepeGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &KingPepeGUI::quitRequested);
...
void KingPepeGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void KingPepeGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void KingPepeGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &KingPepeGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &KingPepeGUI::show);
                e->ignore();
            }
        }
    }
...
void KingPepeGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void KingPepeGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void KingPepeGUI::incomingTransaction(const QString& date, KingPepeUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(KingPepeUnits::formatWithUnit(unit, amount, true));
...
void KingPepeGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void KingPepeGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool KingPepeGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool KingPepeGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void KingPepeGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void KingPepeGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void KingPepeGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void KingPepeGUI::updateProxyIcon()
{
    std::string ip_port;
...
void KingPepeGUI::updateWindowTitle()
{
...
void KingPepeGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void KingPepeGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void KingPepeGUI::detectShutdown()
{
...
void KingPepeGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void KingPepeGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(KingPepeGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = KingPepeGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void KingPepeGUI::subscribeToCoreSignals()
{
...
void KingPepeGUI::unsubscribeFromCoreSignals()
{
...
bool KingPepeGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<KingPepeUnit> units = KingPepeUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const KingPepeUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, KingPepeUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const KingPepeUnit u : KingPepeUnits::availableUnits()) {
        menu->addAction(KingPepeUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(KingPepeUnit newUnits)
{
    setText(KingPepeUnits::longName(newUnits));
}
