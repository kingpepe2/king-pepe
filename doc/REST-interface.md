...
The interface runs on the same port as the JSON-RPC interface, by default port 22093 for mainnet, port 32093 for testnet,
port 322093 for signet, and port 18443 for regtest.
...
See [BIP64](https://github.com/kingpepe/bips/blob/master/bip-0064.mediawiki) for
input and output serialization (relevant for `bin` and `hex` output formats).

Example:
```
$ curl localhost:32093/rest/getutxos/checkmempool/b2cdfd7b89def827ff8af7cd9bff7627ff72e5e8b0f71210f92ea7a4000c5d75-0.json 2>/dev/null | json_pp
{
   "chainHeight" : 325347,
...
Risks
-------------
Running a web browser on the same node with a REST enabled kingpeped can be a risk. Accessing prepared XSS websites could read out tx/block data of your node by placing links like `<script src="http://127.0.0.1:22093/rest/tx/1234567890.json">` which might break the nodes privacy.
