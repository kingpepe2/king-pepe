...
/** Call node.updateRwSetting() with KingPepe 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const util::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because KingPepe 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/kingpepe/kingpepe/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because kingpepe 22.x will not try to read these.
...
//! Convert enabled/size values to kingpepe -prune setting.
...
//! Get pruning enabled value to show in GUI from kingpepe -prune setting.
...
//! Get pruning size value to show in GUI from kingpepe -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayKingPepeUnit")) {
        settings.setValue("DisplayKingPepeUnit", QVariant::fromValue(KingPepeUnit::KPEPE));
    }
    QVariant unit = settings.value("DisplayKingPepeUnit");
    if (unit.canConvert<KingPepeUnit>()) {
        m_display_kingpepe_unit = unit.value<KingPepeUnit>();
    } else {
        m_display_kingpepe_unit = KingPepeUnit::KPEPE;
        settings.setValue("DisplayKingPepeUnit", QVariant::fromValue(m_display_kingpepe_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if kingpepe.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_kingpepe_unit);
...
{
    if (new_unit.isNull() || new_unit.value<KingPepeUnit>() == m_display_kingpepe_unit) return;
    m_display_kingpepe_unit = new_unit.value<KingPepeUnit>();
    QSettings settings;
    settings.setValue("DisplayKingPepeUnit", QVariant::fromValue(m_display_kingpepe_unit));
    Q_EMIT displayUnitChanged(m_display_kingpepe_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/kingpepe/kingpepe/pull/8273
    ...
    // (https://github.com/kingpepe-core/gui/issues/567).
