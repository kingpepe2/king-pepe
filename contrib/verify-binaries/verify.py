...
"""Script for verifying KingPepe Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from kingpepecore.org and kingpepe.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/kingpepe-core/guix.sigs/tree/main/builder-keys
...
HOST2 = "https://kingpepe.org"
VERSIONPREFIX = "kingpepe-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, t.List[SigData], t.List[SigData], t.List[SigData], t.List[SigData]
    ...
    # which pubkeys within the KingPepe community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"kingpepe_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by kingpepecore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the kingpepe release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes kingpepe.org lags behind kingpepecore.org.)')
    )
