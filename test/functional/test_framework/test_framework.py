...
TMPDIR_PREFIX = "kingpepe_func_test_"
...
class KingPepeTestMetaClass(type):
    """Metaclass for KingPepeTestFramework.

    Ensures that any attempt to register a subclass of `KingPepeTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'KingPepeTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("KingPepeTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("KingPepeTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class KingPepeTestFramework(metaclass=KingPepeTestMetaClass):
    """Base class for a kingpepe test script.

    Individual kingpepe test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave kingpepeds and test.* datadir on exit or error")
        ...
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop kingpepeds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use kingpepe-cli instead of RPC for all commands")
    ...
    def setup(self):
        ...
        fname_kingpeped = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "kingpeped" + config["environment"]["EXEEXT"],
        )
        fname_kingpepecli = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "kingpepe-cli" + config["environment"]["EXEEXT"],
        )
        fname_kingpepeutil = os.path.join(
            config["environment"]["BUILDDIR"],
            "src",
            "kingpepe-util" + config["environment"]["EXEEXT"],
        )
        self.options.kingpeped = os.getenv("KINGPEPED", default=fname_kingpeped)
        self.options.kingpepecli = os.getenv("KINGPEPECLI", default=fname_kingpepecli)
        self.options.kingpepeutil = os.getenv("KINGPEPEUTIL", default=fname_kingpepeutil)
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: kingpepeds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("KingPepeRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'kingpeped', self.options.kingpeped) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'kingpepe-cli', self.options.kingpepecli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a kingpeped test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple kingpeped test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as kingpeped's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        if not os.path.isdir(cache_node_dir):
            ...
            self.nodes.append(
                TestNode(
                    ...
                    timewait=self.rpc_timeout,
                    timeout_factor=self.options.timeout_factor,
                    kingpeped=self.options.kingpeped,
                    kingpepe_cli=self.options.kingpepecli,
                    coverage_dir=None,
                    cwd=self.options.tmpdir,
                    descriptors=self.options.descriptors,
                ))
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in kingpepe.conf
    ...
    def skip_if_no_kingpeped_tracepoints(self):
        """Skip the running test if kingpeped has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("kingpeped has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_kingpeped_zmq(self):
        """Skip the running test if kingpeped has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("kingpeped has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if kingpepe-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("kingpepe-wallet has not been compiled")

    def skip_if_no_kingpepe_util(self):
        """Skip the running test if kingpepe-util has not been compiled."""
        if not self.is_kingpepe_util_compiled():
            raise SkipTest("kingpepe-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if kingpepe-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("kingpepe-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether kingpepe-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether kingpepe-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_kingpepe_util_compiled(self):
        """Checks whether kingpepe-util was compiled."""
        return self.config["components"].getboolean("ENABLE_KINGPEPE_UTIL")
