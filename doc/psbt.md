# PSBT Howto for KingPepe Core

Since KingPepe Core 0.17, an RPC interface exists for Partially Signed KingPepe
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/kingpepe/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for KingPepe transactions that are not fully signed
...
#### Multisig with multiple KingPepe Core instances
...
KingPepe Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `kingpepe-cli` is used.
...
Later, when *V* KPEPE has been received on *Amulti*, and Bob and Carol want to
